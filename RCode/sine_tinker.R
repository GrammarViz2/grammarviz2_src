...
p2
write.table(y, "../data/aa_synth/sine_0_a.txt", col.names = F, row.names = F)
#
...
p3
write.table(y, "../data/aa_synth/sine_0_a1.txt", col.names = F, row.names = F)
#
# stretch
# 
points3 <- which(t >= anomaly_pos2[2] & t <= anomaly_pos2[2] + pi * 1.5)
points3_new <- sin( seq(min(points3), max(points3), by = 0.15) )
y <- c( y[1:(min(points3))], points3_new, y[(max(points3)):length(y)])
p4 <- ggplot(data.frame(x = 1:length(y), y = y), aes(x, y)) + geom_line() + ggtitle("Sine wave + anomaly")
p4
write.table(y, "../data/aa_synth/sine_0_a1.txt", col.names = F, row.names = F)
