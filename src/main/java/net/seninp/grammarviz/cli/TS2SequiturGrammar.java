...
public class TS2SequiturGrammar {
  ...
  public static void main(String[] args) throws Exception {
    ...
    new RulesWriter(TS2GrammarParameters.OUT_FILE).write(rules);
  }
  ...
  private static void logParams() {
    String output = "GrammarViz2 CLI converter v.1" + NEWLINE + "parameters:" + NEWLINE
        ...
        + "  Pruning rules:               " + TS2GrammarParameters.PRUNE_RULES + NEWLINE + NEWLINE;

    consoleLogger.info(output);
  }

  private static GrammarRules inferGrammarRules(double[] series, SAXRecords saxData, String str)
      throws Exception {
  ...
  private static SAXRecords discretize(double[] series) throws SAXException {
    ...
    if (TS2GrammarParameters.NUM_WORKERS <= 1) {
      ...
      return psax.process(series, TS2GrammarParameters.NUM_WORKERS,
          TS2GrammarParameters.SAX_WINDOW_SIZE, TS2GrammarParameters.SAX_PAA_SIZE,
          TS2GrammarParameters.SAX_ALPHABET_SIZE, TS2GrammarParameters.SAX_NR_STRATEGY,
          TS2GrammarParameters.SAX_NORM_THRESHOLD);
    }
  }
